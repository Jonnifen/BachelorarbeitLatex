%! Author = jbf
%! Date = 25.07.25

%Dokumentheader
%------------------------------------------------------------------------------------------------------
\documentclass[
pagesize,				% flexible Blattgröße
a4paper,				% DIN A4
oneside,				% einseitig gedruckt
headsepline,		    % Strich unter der Kopfzeile
11pt,					% 11pt Schriftgröße
halfparskip,		    % Europäischer Satz: Abstand zwischen Absätzen
%draft,					%Vorabversion
final,					% Endgültige Version
listof=totoc           % nicht nummerierter eintrag in den Inhalt (verzeichnisse)
]{scrartcl}			    % KOMA_Script-klasse Artikel
%------------------------------------------------------------------------------------------------------
%Dokumenteinstellungen
%------------------------------------------------------------------------------------------------------
% Packages und Grundeinstellungen des Dokumentes wurde ausgelagert
\input{Einstellung/Grundeinstellungen_Jon}
%------------------------------------------------------------------------------------------------------
% Document-Start
%------------------------------------------------------------------------------------------------------
\begin{document}
%Deckblatt der FH als PDF eingefügt
%------------------------------------------------------------------------------------------------------
\includepdf{Deckblatt/Deckblatt Bachelorthesis Jon Feddersen.pdf}
%------------------------------------------------------------------------------------------------------
%Sperrvermerk
%------------------------------------------------------------------------------------------------------
\input{Kapitel/0. Sperrvermerk/Sperrvermerk}
%------------------------------------------------------------------------------------------------------
%Definition von Großen römischen Zahlen für die Kapitel
%------------------------------------------------------------------------------------------------------
\renewcommand{\thesection}{\roman{section}}
\pagenumbering{Roman}
\ohead[\pagemark]{}
%------------------------------------------------------------------------------------------------------
%Einstellung Inhaltsverzeichnis
%------------------------------------------------------------------------------------------------------
\singlespacing
\renewcommand{\contentsname}{Inhaltsverzeichnis}
\phantomsection
\addtocounter{section}{1}
\setcounter{page}{2}
%------------------------------------------------------------------------------------------------------
%Löschen der Einstellungen für Kopfzeile und Fußzeile
%------------------------------------------------------------------------------------------------------
\clearmainofpairofpagestyles
\clearplainofpairofpagestyles
\clearpairofpagestyles
%------------------------------------------------------------------------------------------------------
%Abschnitt und Seitenzahl in Kopfzeile
%------------------------------------------------------------------------------------------------------
\ohead[\pagemark]{\pagemark}
\ihead[\headmark]{\headmark}
%------------------------------------------------------------------------------------------------------
%Inhaltsverzeichnis
%------------------------------------------------------------------------------------------------------
\tableofcontents
\pagebreak
%------------------------------------------------------------------------------------------------------
%Abbildungsverzeichnis
%------------------------------------------------------------------------------------------------------
\listoffigures
\pagebreak
%------------------------------------------------------------------------------------------------------
%Tabellenverzeichnis
%------------------------------------------------------------------------------------------------------
\listoftables
\pagebreak
%------------------------------------------------------------------------------------------------------
%%Verzeichnis der Formelzeichen, Symbole und Indizes
%------------------------------------------------------------------------------------------------------
%\nomenclature{$E$}{Energie}
%\nomenclature{$v$}{Geschwindigkeit}
%\nomenclature{$t$}{Zeit}
%\section*{Verzeichnis der Formelzeichen, Symbole und Indizes}
%\addcontentsline{toc}{section}{Verzeichnis der Formelzeichen, Symbole und Indizes}
%\printnomenclature
%\pagebreak
%------------------------------------------------------------------------------------------------------
%Abkürzungsverzeichnis
%------------------------------------------------------------------------------------------------------
\section*{Abkürzungsverzeichnis}
\addcontentsline{toc}{section}{Abkürzungsverzeichnis} % ins Inhaltsverzeichnis aufnehmen

\begin{acronym}[csrf\_token]

  \acro{AC}{Alternating Current (Wechselstrom)}
  \acro{API}{Application Programming Interface}
  \acro{BIT}{Burn-In-Test}
  \acro{CSS}{Cascading Style Sheets}
  \acro{DC}{Direct Current (Gleichstrom)}
  \acro{DOM}{Document Object Model}
  \acro{DSCP}{Dynamic-Short-Circuit-Protection}
  \acro{DTD}{Document Type Definition}
  \acro{DUT}{Device-Under-Test}
  \acro{DUTs}{Devices-Under-Test}
  \acro{FSW}{Functional-Switching}
  \acro{HTML}{HyperText Markup Language}
  \acro{HTTP}{HyperText Transfer Protocol}
  \acro{JS}{JavaScript}
  \acro{JSON}{JavaScript Object Notation}
  \acro{MySQL}{Relationales Datenbankmanagementsystem}
  \acro{OCP}{Over-Current-Protection}
  \acro{ORM}{Object-Relational Mapping}
  \acro{OTP}{Overtemperature-Protection-Test}
  \acro{QR}{Quick Response (Code)}
  \acro{SAX}{Simple API for XML}
  \acro{SQL}{Structured Query Language}
  \acro{SVG}{Scalable Vector Graphics}
  \acro{URL}{Uniform Resource Locator}
  \acro{UTF-8}{Unicode Transformation Format – 8 Bit}
  \acro{VCE}{Collector-Emitter-Spannung}
  \acro{WSGI}{Web Server Gateway Interface}
  \acro{XML}{Extensible Markup Language}
  \acro{XSD}{XML Schema Definition}
  \acro{XSLT}{Extensible Stylesheet Language Transformations}
  \acro{XXE}{XML External Entity}
  \acro{csrf\_token}{Cross-Site Request Forgery Token (Sicherheitsmechanismus)}
\end{acronym}
\pagebreak
%------------------------------------------------------------------------------------------------------
%Definition für Kopfzeile und arabische Sections
%------------------------------------------------------------------------------------------------------
\renewcommand{\sectionmark}[1]{\markright{#1}}
\renewcommand{\subsectionmark}[1]{}
\renewcommand{\subsubsectionmark}[1]{}
\onehalfspacing
\renewcommand{\thesection}{\arabic{section}}
\setcounter{section}{0}
\pagenumbering{arabic}
\setcounter{page}{1}
%------------------------------------------------------------------------------------------------------
%1. Einleitung
%------------------------------------------------------------------------------------------------------
\input{Kapitel/1. Einleitung/Einleitung}
%------------------------------------------------------------------------------------------------------
%2. Hauptteil
%------------------------------------------------------------------------------------------------------
\input{Kapitel/2. Hauptteil/2. Grundlagen}

\input{Kapitel/2. Hauptteil/3. Analyse und Konzeption}

\input{Kapitel/2. Hauptteil/4. Implementierung}

\input{Kapitel/2. Hauptteil/5. Integration und Test}
%------------------------------------------------------------------------------------------------------
%3. Schluss
%------------------------------------------------------------------------------------------------------
\input{Kapitel/3. Schluss/6. Fazit und Ausblick}
\newpage
%------------------------------------------------------------------------------------------------------
%Einstellungsänderungen der Seitenzahlen zu kleine römisch Zahlen
%------------------------------------------------------------------------------------------------------
\renewcommand{\thesection}{\roman{section}}
\pagenumbering{roman}
\ohead[\pagemark]{}
\setcounter{section}{0}
%------------------------------------------------------------------------------------------------------
%Literaturverzeichnis
%------------------------------------------------------------------------------------------------------
\newpage
\section{Literaturverzeichnis}
\label{sec:literaturverzeichnis}
%\addcontentsline{toc}{section}{Literaturverzeichnis}
\printbibliography
\newpage
%------------------------------------------------------------------------------------------------------
%Anhangsverzeichnis
%------------------------------------------------------------------------------------------------------
\section{Anhangsverzeichnis}
\label{sec:anhangsverzeichnis}
%\section*{Anhangsverzeichnis}
%\addcontentsline{toc}{section}{Anhangsverzeichnis}

\begin{itemize}
  \item[\textbf{A.}] \textbf{XML-Beispielstruktur} \label{anh:xml}
  \item[\textbf{B.}] \textbf{Flask-Routenübersicht} \label{anh:flask}
  \item[\textbf{C.}] \textbf{Testdatenbank-Schema} \label{anh:db}
\end{itemize}

\newpage
%------------------------------------------------------------------------------------------------------
%Erklärung
%------------------------------------------------------------------------------------------------------
\input{Kapitel/6. Erklärung/Erklärung}
%------------------------------------------------------------------------------------------------------
%Dokument-End
%------------------------------------------------------------------------------------------------------
\end{document}
%------------------------------------------------------------------------------------------------------

