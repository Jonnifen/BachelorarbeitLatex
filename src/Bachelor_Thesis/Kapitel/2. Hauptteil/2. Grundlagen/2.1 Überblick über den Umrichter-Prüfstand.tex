...
Die Umrichter werden in der gegebenen Fachliteratur zu der Test-Bench als \ac{DUTs} oder \ac{DUT} bezeichnet.
...
Jeder Test bzw. jedes Testmodul ist autonom und kann mehrmals ausgeführt werden, auch mit unterschiedlichen Parametern.
\\
Im Folgenden wird eine kurze Beschreibung der Funktionen der für diese Arbeit relevanten Testmodule gegeben:

\textbf{Driver-Consumption-Test:}
...
\item Die Umrichter werden auf dem Carrier befestigt. Es werden meist 3 dieser Geräte gleichzeitig getestet, Abweichungen sind je nach Bauform der \ac{DUT} möglich.
...
\item Das Montieren von VCE-Klemmen an die AC-Kontakte der \ac{DUTs} zum Controller des Teststandes.
...
Diese Daten können über das Scannen von vorliegenden \ac{QR}- oder Barcodes eingefügt werden.
...
Nach dem Durchlaufen eines Tests wird automatisch eine \ac{XML}-Struktur mit den erhobenen Messdaten und allen vorher bestimmten Einstellungen in Adminreport generiert.
Diese Struktur wird manuell in eine separate \ac{XML}-Datei eingefügt und so gespeichert.
